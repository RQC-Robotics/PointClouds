...
class TensorReplayBufer():
    def __init__(self, max_size, act_dim, state_dim, device, image_pad=4, enc_type='Linear', aug_flag=False):
        ...
        if enc_type == 'Conv' or enc_type == 'PointCloud':
            self.state_buf = torch.zeros((max_size, *state_dim), dtype=torch.float32, device=self.device)
            self.next_state_buf = torch.zeros((max_size, *state_dim), dtype=torch.float32, device=self.device)
        else:
            self.state_buf = torch.zeros((max_size, state_dim), dtype=torch.float32, device=self.device)
            self.next_state_buf = torch.zeros((max_size, state_dim), dtype=torch.float32, device=self.device)
            
        self.act_buf = torch.zeros((max_size, act_dim), dtype=torch.float32, device=self.device)
        self.rew_buf = torch.zeros(max_size, dtype=torch.float32, device=self.device)
        self.done_buf = torch.zeros(max_size, dtype=torch.float32, device=self.device)
        self.ptr = 0
    ...
    def store(self, state, act, rew, next_state, done):
        index = self.ptr % self.max_size
        self.state_buf[index] = state
        self.act_buf[index] = torch.tensor(act, dtype=torch.float32, device=self.device)
        self.rew_buf[index] = torch.tensor(rew, dtype=torch.float32, device=self.device)
        self.next_state_buf[index] = next_state
        self.done_buf[index] = done
        self.ptr += 1
    ...
    def sample_batch(self, batch_size):
        ...
        states = torch.as_tensor(self.state_buf[idxs], dtype=torch.float32, device='cuda:0')
        # states = self.aug(states)
        actions = torch.as_tensor(self.act_buf[idxs], dtype=torch.float32, device='cuda:0')
        rewards = torch.as_tensor(self.rew_buf[idxs], dtype=torch.float32, device='cuda:0')
        next_states = torch.as_tensor(self.next_state_buf[idxs], dtype=torch.float32, device='cuda:0')
        # next_states = self.aug(next_states)
        dones = torch.as_tensor(self.done_buf[idxs], dtype=torch.float32, device='cuda:0')
