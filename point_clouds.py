...
class PointCloudGenerator(object):
    ...
    def generateCroppedPointCloud(self, save_img_dir=None):
        ...
        for cam_i in self.cam_names:
            ...
            o3d_cloud = o3d.geometry.PointCloud.create_from_depth_image(od_depth, od_cammat, depth_trunc=100)
            ...
            cam_body_id = self.sim.model.cam_bodyid[cam_i]
            cam_pos = self.sim.model.cam_pos[cam_i] + self.sim.model.body_pos[cam_body_id]
            ...
            transformed_cloud.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.1, max_nn=250))
    ...
    def depthimg2Meters(self, depth):
        ...
        near = self.sim.model.vis.map_.znear * extent
        far = self.sim.model.vis.map_.zfar * extent
    ...
    def captureImage(self, cam_ind, capture_depth=True):
        print(self.cam_names[cam_ind])
        rendered_images = self.sim.render(self.img_height, self.img_width, camera_id=self.cam_names[cam_ind], depth=capture_depth)
        if capture_depth:
            depth = rendered_images
